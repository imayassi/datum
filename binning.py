...
def bin(data):
    ...
    # array=df.values
    response = df['ABANDONED']
    dependent = df.drop(['ABANDONED'], axis=1)
    Y = response.values
    X = dependent.values
    x=X[:,0:]
    y=Y
    ...
    for m in reversed(nodes):
        ...
        if np.where(clf.tree_.children_left==m)[0]>=0 and m<right_split: #CHECK IF THIS LEAF BELONGS TO THE LEFT CHILD AND IF IT IS ON LEFT SPLIT OF THE LEFT CHILD OF THE TREE (i.e. LEFT LEFT)
            ...
            bins[right_leaf_value]='between '+ repr(clf.tree_.threshold[m]) + ' and ' +  repr(clf.tree_.threshold[i[0]])
            ...
            left_leaf_value=clf.tree_.children_left[m]
            right_leaf_value=clf.tree_.children_right[m]
            bins[left_leaf_value]='between '+ repr(clf.tree_.threshold[i[0]]) + ' and ' +  repr(clf.tree_.threshold[m])
