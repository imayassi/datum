...
for x in range(150,-1,-1):
    ...
    data="select COUNT (customer_key) as retained from CTG_ANALYTICS_WS.SM_CUSTOMER_RETENTION_BASE_TEST_FULL where tax_year=2014 and tto_flag=1 and core_flag=1 and tax_day_cy<=%d  and risk_flag=1" % x
    df = pd.read_sql(data, conn, coerce_float=False)
    df2=pd.concat([df, df2], ignore_index=True)
