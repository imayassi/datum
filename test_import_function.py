...
for x in range(12,120,1):
    print x
    scoring_data = "SELECT A.* FROM (SELECT * FROM  CTG_ANALYTICS_WS.SM_RETENTION_MODEL  WHERE   TAX_YEAR=2016 AND CUSTOMER_DEFINITION_ADJ='NEW TO TURBOTAX' )A inner join (SELECT DISTINCT CUSTOMER_KEY FROM  CTG_ANALYTICS_WS.SM_RETENTION_MODEL  WHERE   TAX_YEAR=2016 AND TAX_DAY>=112) B ON A.CUSTOMER_KEY=B.CUSTOMER_KEY limit 100"
    data = "SELECT * FROM (SELECT * FROM  CTG_ANALYTICS_WS.SM_RETENTION_MODEL  WHERE   TAX_YEAR<2014 AND TAX_DAY<=150 AND CUSTOMER_DEFINITION_ADJ='NEW TO TURBOTAX'  )A   order by random() limit 1000"
